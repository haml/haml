  ...
  module HamlHelpers
    ...
    module XssMods
      ...
      def html_escape_with_haml_xss(text)
        ...
        Haml::HamlUtil.html_safe(html_escape_without_haml_xss(str))
      end

      # Output is always HTML safe
      def find_and_preserve_with_haml_xss(*args, &block)
        Haml::HamlUtil.html_safe(find_and_preserve_without_haml_xss(*args, &block))
      end

      # Output is always HTML safe
      def preserve_with_haml_xss(*args, &block)
        Haml::HamlUtil.html_safe(preserve_without_haml_xss(*args, &block))
      end

      # Output is always HTML safe
      def list_of_with_haml_xss(*args, &block)
        Haml::HamlUtil.html_safe(list_of_without_haml_xss(*args, &block))
      end

      # Input is escaped, output is always HTML safe
      def surround_with_haml_xss(front, back = front, &block)
        Haml::HamlUtil.html_safe(
      ...
      def precede_with_haml_xss(str, &block)
        Haml::HamlUtil.html_safe(precede_without_haml_xss(haml_xss_html_escape(str), &block))
      end

      # Input is escaped, output is always HTML safe
      def succeed_with_haml_xss(str, &block)
        Haml::HamlUtil.html_safe(succeed_without_haml_xss(haml_xss_html_escape(str), &block))
      end

      # Output is always HTML safe
      def capture_haml_with_haml_xss(*args, &block)
        Haml::HamlUtil.html_safe(capture_haml_without_haml_xss(*args, &block))
      end

      # Input will be escaped unless this is in a `with_raw_haml_concat`
      # block. See #Haml::HamlHelpers::ActionViewExtensions#with_raw_haml_concat.
      ...
      def haml_indent_with_haml_xss
        Haml::HamlUtil.html_safe(haml_indent_without_haml_xss)
      end

      # Output is always HTML safe
      def escape_once_with_haml_xss(*args)
        Haml::HamlUtil.html_safe(escape_once_without_haml_xss(*args))
      end
      ...
      def haml_xss_html_escape(text)
        return text unless Haml::HamlUtil.rails_xss_safe? && haml_buffer.options[:escape_html]
        html_escape(text)
      end
    end
